...
| TT.8 | Weekend detrás del toggle Extended · doc confirmada `transcript:109` McKinley *"I'm not going to tell them on Saturday they need to go in"* + `sot.md:61` "Standard 40h/week" (5-day) · default My Timesheet ahora **Mon-Fri, 7am-7pm** · toggle "Weekend + off-hours" expande a Mon-Sun, 5am-11pm · totals summary sigue contando weekend entries (capacity check) · `dayCount` computed 5/7 · `visibleDays = week.slice(0, dayCount)` · grid template cols dinámico · Team View **NO cambia** (manager mantiene 7-day siempre para overtime detection) · toolbar button relabeled ("Standard 5-day" / "Weekend + off-hours") · **hotfix**: day columns iteran `visibleDays` (era `week` de 7 · crash "undefined.map") | `src/components/timetracker/WeeklyGrid.tsx` (dayCount + visibleDays + positionedByDay + grid template dinámico + button label + day columns fix) |
| TT.9 | 3 features de gestión de entries reportados por Diego: (a) **Reset week** button en toolbar (RotateCcw icon, destructive tone) con confirm dialog "Reset this week? All N entries will be deleted" · disabled si no hay entries · (b) **Reset day** button hover-reveal en el day header (Trash2 icon, esquina superior-derecha) con confirm dialog "Reset Mon 7? All N entries for this day will be deleted" · solo aparece si el día tiene entries · (c) **Delete entry inline** hover-reveal en EntryBlock (Trash2 top-right, junto al deliverable badge · shift left cuando hay ✓) con confirm dialog verbose "Delete this entry? {label} · {hours}h · {project} · {timeRange}" · sin abrir el TimeEntryForm · (d) **Copy previous week de-duplicación**: track `alreadyCopiedThisWeek` (entries con id `TE-COPY-*` en la current week+designer) · botón disabled + label "Already copied" + tooltip "Already copied · reset the week to copy again" · previene doble-click accidental que duplicaba entries | `src/components/timetracker/WeeklyGrid.tsx` (4 nuevos props · toolbar Reset button · day header Trash button · EntryBlock delete button · Copy button disabled state) · `src/TimeTracker.tsx` (handleResetWeek + handleResetDay handlers · alreadyCopiedThisWeek useMemo · wire onDeleteEntry a handleDelete existente) |
